...
std::map<RichImage*, std::vector<cv::DMatch>> multiMatch(std::vector<RichImage*> models, RichImage* target,
	...
	///HARD MODELS
	std::vector<int> totalMatches(models.size());

	std::vector<cv::Point> positions(models.size());

	std::vector<std::vector<cv::DMatch>> matches(models.size());
	
	std::vector<bool> isAlive(models.size());
